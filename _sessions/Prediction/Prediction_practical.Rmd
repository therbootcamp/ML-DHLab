...
5. Using `trainControl()`, create `ctrl_none` object setting `method = "none"`.
...
2. Print the datasets to get an idea of their contents. 
...
6. Contemplate, if you had to use one of these three models in the real-world, which one would it be?
...
2. Using `train()`, fit a regression model called `price_glm` predicting the house sale price (`price`) using all features in `house_train`. Specifically:
...
5. Using `train()`, fit a decision tree model called `price_rpart` predicting `price` using all features in `house_train`. Specifically:
...
6. Explore your `price_rpart` object by looking at `price_rpart$finalModel` and plotting it with `plot(as.party(price_rpart$finalModel))`. What do you find?
...
7. Using `predict()` save the fitted values of `price_rpart` object as `rpart_fit`.
...
8. Using `train()`, fit a random forest model called `price_rf` predicting `price` using all features in `house_train`. Specifically:

- for the `form` argument, use `price ~ .`.
- for the `data` argument, use  `house_train` in the data argument.
- for the `method` argument, use `method = "rf"` to fit random forests.
- for the `trControl` argument, use your `ctrl_none` object you created before.
- for the `mtry` parameter, use `mtry` = 2. 
...
10. Save the true training criterion values as a vector called `criterion_train`.
...
11. Using `postResample()`, determine the fitting performance of each of your models separately. 
...
12. Which model had the best fit? Same model as for the graduation data?
