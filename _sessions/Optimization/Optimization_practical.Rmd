...
1. You will again begin by analyzing the graduation data. Read in the datasets `graduation_train.csv` and `graduation_test.csv` and convert all character to factors. Furthermore, the code below limits the number of training cases to 100 to emphasize the impact of the model tuning. 
...
9. Now fit a lasso regression model predicting `Grad.Rate` as a function of all features. Set `alpha = 0` for the Lasso penalty and add `lambda = lambda_vec` as above. 
...
10. Print your `graduation_lasso` object. Which lambda was selected as best performing?

```{r}
graduation_lasso
```

11. Plot your `graduation_lasso` object. What do you see? Does this match the plot match the value identified in the previous task?
...
12. What were your final regression model coefficients for the best lambda value? Find them by running the following code.
...
13. How do these coefficients compare to what you found for the regular and ridge regression? Have some features between set to 0?
...
14. Store the training data and test data criterion (`Grad.Rate`) as `criterion_train` and `criterion_test`. 
...
15. Using `predict()`, save the fitted values of your models as  `glm_fit`, `ridge_fit`, and `lasso_fit`. 
...
16. Using `postResample` evaluate the fitting performance of your models. Which model has the best performance in fitting the training data? 
...
16. Using `predict()` and `newdata = college_test`, save the predicted values of your models as  `glm_pred`, `ridge_pred`, and `lasso_pred`. 
...
17. Using `postResample` evaluate the prediction performance of your models. Which model has the best performance in predicting the test data? Did the regularized regressions outperform the unregularized one? 
...
16. Using `postResample` evaluate the fitting performance of your models. Which model has the best performance in fitting the training data? If you like compare to the gression models of the previous section. 
...
16. Using `predict()` and `newdata = college_test`, save the predicted values of your models as `rpart_pred`, and `rf_pred`. 
...
17. Using `postResample` evaluate the prediction performance of your models. Which model has the best performance in predicting the test data? Did the tree models outperform the regularized regressions?
...
### Z - Challenges: Explore tuning parameter grids
